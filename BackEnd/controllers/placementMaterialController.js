  ...
  try {
    ...
    if (!title || !description || !category || !materialType) {
      return res
        ...
        .json({
          message:
            "Title, description, category, and materialType are required.",
        });
    }
    ...
    if (materialType === "Link" && !resourceUrl) {
      return res
        .status(400)
        .json({ message: "Resource URL is required for Link type." });
    }
    ...
    if (req.file) {
      const uploaded = await uploadImageCloudinary(
        req.file,
        "PlacementMaterials",
      );
...
export const getMaterialById = async (req, res) => {
  try {
    const material = await PlacementMaterial.findById(req.params.id).populate(
      "postedBy",
      "name profile",
    );
...
export const toggleUpvote = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ upvotes: material.upvotes.length, upvoted: !alreadyUpvoted });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
