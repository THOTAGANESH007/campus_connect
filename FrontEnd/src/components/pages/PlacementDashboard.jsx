...
import {
  getOverview, getByCompany, getByBranch, getStatusDistribution,
} from "../../services/profileService";
import { useAuth } from "../../context/AuthContext";
import { Link } from "react-router-dom";
import {
  Users, Briefcase, CheckCircle, TrendingUp, BarChart2, PieChart, ArrowLeft,
} from "lucide-react";
...
export default function PlacementDashboard() {
  ...
  useEffect(() => {
    Promise.all([getOverview(), getByCompany(), getByBranch(), getStatusDistribution()])
      .then(([ov, comp, br, st]) => {
        setOverview(ov);
        setByCompany(comp);
        setByBranch(br);
  ...
  const statCards = overview ? [
    { label: "Total Students", value: overview.totalStudents, icon: Users, color: "indigo" },
    { label: "Active Drives", value: overview.totalDrives, icon: Briefcase, color: "purple" },
    { label: "Applications", value: overview.totalApplications, icon: TrendingUp, color: "amber" },
    { label: "Selections", value: overview.selectedCount, icon: CheckCircle, color: "emerald" },
  ] : [];
      ...
      <div className="bg-gradient-to-br from-slate-900 via-indigo-950 to-black text-white py-14 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="text-4xl font-black mt-2 flex items-center gap-3">
            <BarChart2 size={32} className="text-indigo-300" /> Placement Analytics
          </h1>
          <p className="text-slate-400 mt-2">Real-time placement statistics and insights</p>
        </div>
      </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              ...
              {byCompany.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
                  <h2 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                    <BarChart2 size={18} className="text-indigo-500" /> Placements by Company
                  </h2>
              ...
              {byBranch.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
                  <h2 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                    <PieChart size={18} className="text-purple-500" /> Placements by Branch
                  </h2>
            ...
            {statusDist.length > 0 && (
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
                <h2 className="text-lg font-bold text-slate-900 mb-6">Application Status Distribution</h2>
                    ...
                    {statusDist.map((d, i) => {
                      ...
                      return (
                        <div key={d._id} className="flex items-center gap-3">
                          <span className="w-3 h-3 rounded-full" style={{ background: ["#6366f1","#f59e0b","#22c55e","#ef4444"][i] }} />
                          <span className="text-sm font-semibold text-slate-700 w-24">{d._id}</span>
                          <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: ["#6366f1","#f59e0b","#22c55e","#ef4444"][i] }} />
                          </div>
                          <span className="text-sm text-slate-500 font-medium w-10 text-right">{pct}%</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            {byCompany.length === 0 && byBranch.length === 0 && (
              <div className="text-center py-20 bg-white rounded-2xl border border-slate-100">
                ...
                <h3 className="text-xl font-bold text-slate-700">No placement data yet</h3>
                <p className="text-slate-400 mt-2">Data will appear as students apply and get selected.</p>
              </div>
            )}
          </>
        )}
