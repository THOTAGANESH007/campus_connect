...
import {
  User, Mail, Phone, GraduationCap, Briefcase, Upload,
  Save, CheckCircle, BookOpen, Star, FileText,
} from "lucide-react";
...
export default function StudentProfile() {
  ...
  const completeness = Math.round((Object.values(filledProfile).filter(Boolean).length / fields.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16">
      {/* Hero */}
      <div className="bg-gradient-to-br from-indigo-900 via-slate-900 to-black text-white py-16 px-6">
        <div className="max-w-4xl mx-auto">
          <Link to="/drives" className="text-indigo-300 hover:text-white text-sm mb-6 inline-block">← Back to Drives</Link>
          ...
          <div className="mt-8 bg-white/5 rounded-2xl p-5 border border-white/10">
            <div className="flex justify-between mb-2">
              <span className="text-sm font-semibold text-slate-300">Profile Completeness</span>
              ...
              {fields.map((f) => (
                <span
                  ...
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                    filledProfile[f] ? "bg-green-500/20 text-green-300" : "bg-white/5 text-slate-400"
                  }`}
                >
                  {filledProfile[f] ? "✓" : "○"} {f}
                </span>
      ...
      <div className="max-w-4xl mx-auto px-6 mt-10 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
          ...
          {[
            { label: "Full Name", name: "name", icon: User, type: "text", placeholder: "Your full name" },
            { label: "Phone", name: "phone", icon: Phone, type: "tel", placeholder: "10-digit phone" },
