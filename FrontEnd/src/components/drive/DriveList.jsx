...
const DriveList = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans selection:bg-indigo-500/20">
      ...
      <div className="relative z-10 max-w-350 mx-auto p-6 lg:p-10">
        {/* Header Section */}
        <motion.div
        ...
        {loading ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="h-64 bg-white/50 rounded-3xl animate-pulse"
              ></div>
            ))}
          </div>
        ) : error ? (
          ...
          >
            {drives.map((drive) => (
              <motion.div
                ...
                className="group relative bg-white rounded-4xl p-1 shadow-[0_2px_20px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.1)] transition-all duration-300 border border-slate-100"
              >
                <div className="absolute inset-x-0 h-px w-full bg-linear-to-r from-transparent via-indigo-500 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 top-0"></div>
                ...
                <div className="bg-white rounded-[1.8rem] p-7 h-full flex flex-col relative overflow-hidden z-0">
                  ...
                  <div className="mt-auto grid grid-cols-5 gap-3 pt-6 border-t border-slate-50 z-10">
                    ...
                    >
                      <Bookmark
                        size={18}
                        fill={savedIds.has(drive._id) ? "currentColor" : "none"}
                      />
                    </button>
                    {canManageDrives && (
                      <Link
