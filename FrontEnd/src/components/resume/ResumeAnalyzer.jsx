...
function ScoreRing({ score }) {
  ...
  const color = score >= 75 ? CLR.green : score >= 50 ? CLR.yellow : CLR.red;
  ...
  return (
    ...
    >
      ...
      <span style={{ color, fontSize: 13, fontWeight: 600, letterSpacing: 1 }}>
        {label.toUpperCase()}
      </span>
    </div>
  );
...
export default function ResumeAnalyzer() {
  ...
  const s = {
    ...
    errorBox: {
      background: CLR.redSoft,
      border: `1px solid ${CLR.red}44`,
      borderRadius: 12,
      padding: "12px 16px",
      color: CLR.red,
      fontSize: 14,
    ...
    suggBullet: {
      ...
      height: 22,
      borderRadius: "50%",
      background: CLR.yellowSoft,
      border: `1px solid ${CLR.yellow}44`,
      display: "flex",
      alignItems: "center",
    ...
    <div style={s.page}>
      ...
      <div style={s.center}>
        ...
        <div style={s.hero}>
          ...
          <p style={s.heroSub}>
            Upload your resume and get an instant ATS score, skill extraction,
            and personalised suggestions powered by Gemini AI.
          </p>
        </div>
              ...
              {!file ? (
                <>
                  ...
                  <button
                    style={{
                      background: CLR.accentSoft,
                      border: `1px solid ${CLR.accent}66`,
                      color: CLR.accent,
                      borderRadius: 8,
                      padding: "8px 18px",
                      cursor: "pointer",
        ...
        {loading && (
          <div style={s.loader}>
            ...
            <p style={{ color: CLR.muted, fontSize: 14 }}>
              Extracting skills and generating insights…
            </p>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ ...s.grid2, marginBottom: 16 }}>
              ...
              <Card title="ATS Score" icon="🎯">
                ...
                >
                  Score is based on skills, formatting, sections, and keyword
                  density.
                </p>
              </Card>
                ...
                {result.recommendedRoles?.length ? (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {result.recommendedRoles.map((r, i) => (
                      <RoleBadge key={i} label={r} />
                    ))}
                  </div>
                ) : (
                  <p style={{ color: CLR.muted, fontSize: 13 }}>
                    No roles detected.
                  </p>
                )}
              </Card>
            ...
            <div style={{ marginBottom: 16 }}>
              <Card
                title={`Detected Skills (${result.skills?.length ?? 0})`}
                icon="⚡"
              >
                {result.skills?.length ? (
                  <div style={s.tagsWrap}>
                    {result.skills.map((sk, i) => (
                      <SkillTag key={i} label={sk} />
                    ))}
                  </div>
                ) : (
                  <p style={{ color: CLR.muted, fontSize: 13 }}>
                  ...
                  result.suggestions.map((s, i) => (
                    <div
                      ...
                      style={{
                        ...
                        gap: 10,
                        marginBottom:
                          i < result.suggestions.length - 1 ? 12 : 0,
                        fontSize: 14,
                        lineHeight: 1.6,
                        color: CLR.text,
                      }}
            ...
            <button
              style={s.resetBtn}
              onMouseEnter={(e) => {
                e.target.style.borderColor = CLR.accent;
                e.target.style.color = CLR.accent;
              }}
              onMouseLeave={(e) => {
                e.target.style.borderColor = CLR.border;
                e.target.style.color = CLR.muted;
              }}
              onClick={reset}
            >
